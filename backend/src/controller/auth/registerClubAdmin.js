...
export const registerClubAdmin = async (req, res) => {
  try {
    ...
    if (existingUser) {
      ...
      if (currentRoles.includes("club.admin")) {
        return res
          .status(HttpStatusCode.CONFLICT)
          .json(
            new ApiError(HttpStatusCode.CONFLICT, `Username already registered as club.admin`)
          );
      }

      // User exists but doesn't have this role - add the role
      const updatedRoles = [...currentRoles, "club.admin"];
      ...
      return res.status(HttpStatusCode.OK).json(
        new ApiResponse(HttpStatusCode.OK, {
          message: `Role club.admin added successfully`,
          user: {
    ...
    // For club.admin: store username, email is NULL
